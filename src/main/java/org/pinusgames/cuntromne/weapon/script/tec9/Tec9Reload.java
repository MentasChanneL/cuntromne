...
public class Tec9Reload extends Script {
    ...
    public boolean step() {
        ...
        if( this.frame == -1 ) {
            ...
            this.next = false;
            this.animation = Animations.animations.get("tec9.reload");
            this.frame = 0;
        ...
        if(this.frame == 12 && !this.next) {
            this.data.player.getWorld().playSound(this.data.player.getLocation(), "ctum:tec9.out", 1, 1);
        }
        if(this.frame == 22 && this.next) {
            this.data.player.getWorld().playSound(this.data.player.getLocation(), "ctum:tec9.in", 1, 1);
        }
