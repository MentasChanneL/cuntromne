...
public class USPReload extends Script {
    ...
    public boolean step() {
        ...
        if( this.frame == -1 ) {
            ...
            this.next = false;
            this.animation = Animations.animations.get("usp.reload");
            this.frame = 0;
        ...
        if(this.frame == 10 && !this.next) {
            this.data.player.getWorld().playSound(this.data.player.getLocation(), "ctum:weapon.usp.out", 1, 1);
        }
