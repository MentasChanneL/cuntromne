...
public class M4A1Reload extends Script {
    ...
    public boolean step() {
        ...
        if( this.frame == -1 ) {
            ...
            this.next = false;
            this.animation = Animations.animations.get("m4a1.reload");
            this.frame = 0;
        ...
        if(this.frame == 4 && this.next) {
            this.data.player.getWorld().playSound(this.data.player.getLocation(), "ctum:weapon.m4a1.out", 1, 1);
        }
        if(this.frame == 16 && this.next) {
            this.data.player.getWorld().playSound(this.data.player.getLocation(), "ctum:weapon.m4a1.in", 1, 1);
        }
