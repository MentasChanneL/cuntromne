...
public class GrenadeFlash implements WeaponActions{
    ...
    public void fire(WeaponData data) {
        ...
        ProjectileCreator.createFLASH(data.player.getEyeLocation(), data.player.getUniqueId(), 15);
    ...
    public void review(WeaponData data) {
        if(data.player.getCooldown( data.item.getType() ) > 0) return;
        data.player.setCooldown( data.item.getType(), 1 );
        Snowball snowball = ProjectileCreator.createFLASH(data.player.getEyeLocation(), data.player.getUniqueId(), 15);
    ...
    public static ItemStack give(Player player) {
        ...
        meta.setCustomModelData(603);
        meta.displayName(Component.text("*!* МАРШ ВСТАВЬ НАЗВАНИЕ *!*").decoration(TextDecoration.ITALIC, false));
        result.setItemMeta(meta);
        int id = Cuntromne.getInstance().getWeaponID();
        WeaponData data = new WeaponData(new GrenadeFlash(), result, 0, 0, id);
        data.player = player;
